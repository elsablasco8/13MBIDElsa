...
def process_data(datos_creditos: str = "data/raw/datos_creditos.csv",
    ...
    # o HTML con ydata-profiling.
        ####################################################################
    # EXTRA: Generación de reporte de perfilado de datos
    ####################################################################
    try:
        ...
        profile = ProfileReport(
            df_integrado,
            title="Reporte de Datos Integrados",
            explorative=True
        )

        profile.to_file(output_dir + "reporte_datos.html")

        print("Reporte generado correctamente en HTML")

    except ImportError:
        print("ydata-profiling no está instalado. Ejecuta: pip install ydata-profiling")
    ##################################################################################
