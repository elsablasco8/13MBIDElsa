...
def test_great_expectations():
    ...
    add_expectation(
        "nivel_educativo", # Verificar que la situación de vivienda sea una de las categorías válidas
        df_tarjetas["nivel_educativo"].isin(["UNIVERSITARIO_COMPLETO", "UNIVERSITARIO_INCOMPLETO","SECUNDARIO_COMPLETO","DESCONOCIDO","POSGRADO_COMPLETO", "POSGRADO_INCOMPLETO"]).all(), # La validación a realizar
        "El nivel educativo no se encuentra en el rango válido."
    )

    add_expectation(
        "estado_civil ", # Verificar que la situación de vivienda sea una de las categorías válidas
        df_tarjetas["estado_civil"].isin(["UNIVERSITARIO_COMPLETO","UNIVERSITARIO_INCOMPLETO","SECUNDARIO_COMPLETO","DESCONOCIDO","POSGRADO_INCOMPLETO","POSGRADO_COMPLETO"]).all(), # La validación a realizar
        "El estado civil no se encuentra en el rango válido."
    )
    
    ##############################################################################  estado_cliente       10127 non-null  object 
